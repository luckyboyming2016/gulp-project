    ...
    List.prototype = {
        ...
        render: function(data) {
            ...
            if (data && data.apistatus === 1 && data.result) {
                if (this.getLastData(data.result)) {
                    if ((!this.renderTimes && !data.result[this.keyName]) ||
                        (!this.renderTimes && data.result[this.keyName] && !data.result[this.keyName].length)) {
                        data.result.complete_msg = '暂无中奖用户';
                    } else if (parseInt(data.result.nextCursor, 10) != -1) {
                        data.result.complete_msg = '以上为最新200条记录';
                        data.result.nextCursor = -1;
                    } else {
                        data.result.complete_msg = '以上是全部中奖名单';
                    }
                }
