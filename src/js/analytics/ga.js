...
(function(){
    ...
    var Registry = function(){
            ...
            oThis.getUTMU = function(){
                for(var arr_utmu = [], i = 0; i < _registry.length; i++){
                    if(_registry[i]){
                        arr_utmu[Math.floor(i/6)] ^= 1 << i % 6;
                    }
                }
                for(i = 0; i < arr_utmu.length; i++){
                    arr_utmu[i] = _alphabet.charAt(arr_utmu[i] || 0);
                }
                return arr_utmu.join("");
            };
	    },
	    /**
	     * 函数注册表对象实例。
	...
	var Global = function(win, doc){
	     ...
	     * 全局工具对象实例。
	     */
	    $Global = new Global(window, document);
        /**
         * 创建对象方法绑定器。
        ...
        Pick = function(map, key, separator){
            ...
            if(!IsEmpty(map) && !IsEmpty(key) && !IsEmpty(separator)){
                ...
                if(idx > -1){
                    ...
                    result = map.substring(idx + key.indexOf(_equal_) + 1, endIdx);
                }
            }
            return result;
        },
        ...
        Config = function(){
            ...
            function parseOrganicData(data){
                ...
                for(var item, i = 0; i < data.length; i++){
                    item = data[i].split(":");
                    organic.push(new Organic(item[0], item[1]));
                }
                return organic;
            }
            var oThis = this;
    ...
    var Cookie = function(config){
            ...
            function pick(cookie, param, namespace, ignoreExpires){
                ...
                if(!IsEmpty(value)){
                    var idx = value.indexOf("^" + namespace + ".");
                    if(idx < 0){
                        return["", 0];
                    }
                    value = value.substring(idx + namespace.length + 2);
                    if(value.indexOf("^") > 0){
                        value = value.split("^")[0];
                    }
            ...
            */
            function combine(value, namespace){
                return "^" + [[namespace, value[1]].join("."), value[0]].join(":");
            }
            /**
            ...
            oThis.getCookieByNamespace = function(cookie, namespace){
                for(var arrParams = [], value, i = 0; i < params.length; i++){
                    value = pick(cookie, params[i], namespace)[0];
                    IsEmpty(value) || (arrParams[arrParams.length] = params[i] + value + ";");
                }
            ...
            oThis.setCookie = function(key, value, timeout){
                ...
                if(_config.namespace){
                    value = oThis.setCookieByNamespace($Global.document.cookie, key, _config.namespace, value, timeout);
                    key = "2" + key;
            ...
            oThis.setCookieByNamespace = function(cookie, key, namespace, value, timeout){
                ...
                if(!IsEmpty(_value)){
                    cookie = combine(pick(cookie, key, namespace, true), namespace);
                    _value = _value.split(cookie).join("");
    ...
    var GaCookie = function(config){
        ...
        oThis.checkParamsHash = function(){
            return undefined == _utmParamsHash || _utmParamsHash == oThis.hashUTMParams();
        };
        /**
         * 获取Cookie对象的内容。
        ...
        oThis.setUTMZ = function(utmz){
            arrUTMZ = split(utmz);
            for(var i = 0; i < arrUTMZ.length; i++){
        ...
        oThis.initialize = function(){
            ...
            _utmx = undefined;
            _arrUTMV = [];
            _utmParamsHash = undefined;
        };
        oThis.hashUTMParams = function(){
            for(var params = "", i = 0; i < oThis.UTMParams.length; i++){
        ...
        oThis.setDomainHashByCookie = function(domainHash){
            ...
            if(cookie){
                ...
                oThis.setUTMParamsHash(String(oThis.hashUTMParams()));
                result = true;
            }
            return result;
        };
        ...
        oThis.copyCookiePath = function(domainHash, cookiePath){
            ...
            for(var i = 0; i < _utmParams.length; i++){
    ...
    var google_analytics_domain = "https:" == $Global.document.location.protocol ? "https://ssl.google-analytics.com/" : "http://www.google-analytics.com/",
        ...
        Ajax = function(){
            ...
            oThis.send = function(url, param, mark, callback, _ioo){
                if(param.length <= 2036 || _ioo){
                    ...
                    $Global.isFirefox() ? oThis.sendByImage(url + "?" + mark + "&err=ff2post&len=" + param.length, callback) : oThis.Send(param, callback);
                }else{
                    oThis.sendByImage(url + "?" + mark + "&err=len&max=8192&len=" + param.length, callback);
                }
            };
            /**
             * 使用图片对象发出请求。
            ...
            oThis.sendByRequest = function(param, callback){
                var request,
                    Request = $Global.window.XDomainRequest;
                if(Request){
                    request = new Request;
                    request.open("POST", google_analytics_path);
                }else if(Request = $Global.window.XMLHttpRequest){
                    Request = new Request;
                    if("withCredentials"in Request){
                        request = Request;
                        request.open("POST", google_analytics_path, true);
            ...
            oThis.sendByIFrame = function(param, callback){
                ...
                if(doc.body){
                    ...
                    var _loca = doc.location;
                    _loca = _loca.protocol + "//" + _loca.host + "/favicon.ico";
                    _loca = google_analytics_domain + "u/post_iframe.html#" + encodeURIComponent(_loca);
                    var setFra = function(){
                        fra.src = "";
                        fra.parentNode && fra.parentNode.removeChild(fra);
                    };
                    AddListener($Global.window, "beforeunload", setFra);
                    var succes = false,
                        i = 0,
                        fraLoad = function(){
                            if(!succes){
                                try{
                                    if(i > 9 || fra.contentWindow.location.host == doc.location.host){
                                        succes = true;
                                        setFra();
                                        ...
                                        if(win.removeEventListener){
                                            win.removeEventListener(_beforeunload, _setFra, false); }
                                        else{
    ...
    var GaAjax = function(config){
        ...
        oThis.getGoogleAnalyticsPath = function(){
            return"https:" == $Global.document.location.protocol ? "https://ssl.google-analytics.com/__utm.gif" : "http://www.google-analytics.com/__utm.gif";
        };
        /**
         * 发出请求。
         * @param param {String} 发送请求的参数串。
        ...
        oThis.send = function(param, account, domainHash, e, d, callback){
            ...
            var arr_utmb = _gaCookie.getUTMB().split(".");
            if(arr_utmb[1] < 500 || e){
                if(d){
                    ...
                    if(o >= 1){
                        arr_utmb[2] = Math.min(Math.floor(arr_utmb[2] * 1 + o), _config.Qb);
                        arr_utmb[3] = _time;
                    }
                }
                if(e || !d || arr_utmb[2] >= 1){
                    ...
                    if(0 == localServerMode || 2 == localServerMode){
                        ...
                        _ioo && ajax.send(_config.localGifPath, param, mark, callback, true);
                    }
            ...
            _gaCookie.setUTMB(arr_utmb.join("."));
        ...
        oThis.getCheckCode = function(domainHash){
            for(var _params_hash = [], params = [_param_utma, _param_utmz, _param_utmv, _param_utmx], cookie = _gaCookie.getCookie(), _domainHash, i = 0; i < params.length; i++){
                _domainHash = Pick(cookie, params[i] + domainHash, ";");
                if(!IsEmpty(_domainHash)){
                    if(params[i] == _param_utmv){
                        _domainHash = _domainHash.split(domainHash + ".")[1].split("|")[0];
                        if(IsEmpty(_domainHash)){
                            continue;
                        }
            ...
            return Encode(_params_hash.join("+"));
        };
    };
         /**
          * 电子商务网站交易跟踪器。
   ...
   /**
    ...
    GaEComm.Item = function(orderId, sku, name, category, price, quantity){
        ...
        oThis.serialize = function(){
            return "&" + ["utmt=item", "tid=" + Encode(oThis.orderId), "ipc=" + Encode(oThis.sku), "ipn=" + Encode(oThis.name), "iva=" + Encode(oThis.category), "ipr=" + Encode(oThis.price), "iqt=" + Encode(oThis.quantity)].join("&utm");
        };
    };
    /**
     * 使用特定的值创建交易对象。与 _addItem() 一样，此方法仅处理交易跟踪，不提供其他电子商务功能。
    ...
    GaEComm.Transaction = function(orderId, affiliation, total, tax, shipping, city, state, country){
       ...
       /**
         ...
         */
        oThis.serialize = function(){
            return "&" + ["utmt=tran", "id=" + Encode(oThis.orderId), "st=" + Encode(oThis.affiliation), "to=" + Encode(oThis.total), "tx=" + Encode(oThis.tax), "sp=" + Encode(oThis.shipping), "ci=" + Encode(oThis.city), "rg=" + Encode(oThis.state), "co=" + Encode(oThis.country)].join("&utmt");
        };
    };
        /**
         * 客户端信息收集对象。
    ...
    var Client = function(detectFlash){
        ...
        oThis.charset = Encode($Global.document.characterSet ? $Global.document.characterSet : $Global.document.charset ? $Global.document.charset : _empty);
        ...
        oThis.getClientInfo = function(){
            return _amp_ + "utm" + ["cs=" + Encode(oThis.charset), "sr=" + oThis.screen, "sc=" + oThis.colorDepth, "ul=" + oThis.language, "je=" + oThis.javaEnabled, "fl=" + Encode(oThis.flashVersion)].join("&utm");
        };
        /**
         * 获取客户端信息的哈希值。
        ...
        oThis.hashClientInfo = function(){
            var navigator = $Global.window.navigator,
                history_length = $Global.window.history.length;
            navigator= navigator.appName + navigator.version + oThis.language + navigator.platform + navigator.userAgent + oThis.javaEnabled + oThis.screen + oThis.colorDepth + ($Global.document.cookie ? $Global.document.cookie : "") + ($Global.document.referrer ? $Global.document.referrer : "");
    ...
    var SourceTracker = function(domainHash, processedSource, timestamp, config){
        ...
        oThis.getOrganicSource = function(){
            ...
            if(!(IsEmpty(processedSource) || "0" == processedSource || !Contains(processedSource, "://") || oThis.isGoogleCustomSearch(processedSource))){
                ...
                for(var i = 0; i < organic.length; i++){
                    _organic = organic[i];
                    if(Contains(key, _organic.domain.toLowerCase())){
                        processedSource = processedSource.split("?").join(_amp_);
                        if(Contains(processedSource, _amp_ + _organic.key + _equal_)){
                            key = processedSource.split(_amp_ + _organic.key + _equal_)[1];
                            if(Contains(key, _amp_)){
                                key = key.split(_amp_)[0];
                            }
                            return new SourceTracker.Source(undefined, _organic.domain, undefined, "(organic)", "organic", key, undefined);
                        }
                    }
                }
            }
        ...
        oThis.isIgnoredOrganic = function(source){
            ...
            if(source && "organic" == source.medium){
                source = Decode(source.term).toLowerCase();
                for(var i = 0; i < ignoredOrganic.length; i++){
                    result = result || ignoredOrganic[i].toLowerCase() == source;
                }
            }
            return result;
        };
        ...
        oThis.getReferralSource = function(){
            var processedSource = "",
                ...
                processedSource = oThis.processedSource;
            if(!(IsEmpty(processedSource) || "0" == processedSource || !Contains(processedSource, "://") || oThis.isGoogleCustomSearch(processedSource))){
                processedSource = processedSource.split("://")[1];
                if(Contains(processedSource, "/")){
                    content = processedSource.substring(processedSource.indexOf("/"));
                    content = content.split("?")[0];
                    processedSource = processedSource.split("/")[0].toLowerCase();
                }
                if(0 == processedSource.indexOf("www.")){
                    processedSource = processedSource.substring(4);
                }
                return new SourceTracker.Source(undefined, processedSource, undefined, "(referral)", "referral", undefined, content);
            }
        };
        /**
         * 解析带有锚点信息的URL地址。
        ...
        oThis.parseHasAnchorUrl = function(url){
            ...
            if(_config.allowAnchor){
                result = url && url.hash ? url.href.substring(url.href.indexOf("#")) : "";
        ...
        oThis.getDirectSource = function(){
            return new SourceTracker.Source(undefined, "(direct)", undefined, "(direct)", "(none)", undefined, undefined);
        };
        /**
         * 检查是否是忽略的推荐网址。
        ...
        oThis.isIgnoredReferral = function(source){
            ...
            if(source && "referral" == source.medium){
                source = Encode(source.source).toLowerCase();
                for(var i = 0; i < ignoredReferral.length; i++){
                    result = result || Contains(source, ignoredReferral[i].toLowerCase());
                }
            }
            return result;
        };
        ...
        oThis.check = function(source){
            return undefined != source && source.checkSelfIntegrity();
        };
        /**
         * 检查参数_utmz。
         */
        oThis.checkUTMZ = function(cookie){
            ...
            var arrUTMZ = _utmz.split(".");
            var _b_source = new SourceTracker.Source;
            _b_source.initialize(arrUTMZ.slice(4).join("."));
            if(!oThis.check(_b_source)){
                return true;
            }
            var url = $Global.document.location;
            ...
            return oThis.check(_a_source) && _b_source.serialize().toLowerCase() != _a_source.serialize().toLowerCase();
        };
        oThis.dc = function(gaCookie, c){
            if(_config.campaignTrack){
                ...
                if(gaCookie){
                    ...
                    url = oThis.parseHasAnchorUrl($Global.document.location);
                    ...
                    if(oThis.check(_source)){
                        ...
                        if(!b || c){
                            ...
                            gaCookie.setUTMZ([_domainHash, oThis.timestamp, cookie, q, _source.serialize()].join("."));
                            gaCookie.setUTMZCookie();
                        }
                    }
                }
    ...
    SourceTracker.Source = function(id, source, clid, campaign, medium, term, content){
        ...
        oThis.serialize = function(){
            ...
            return $Global.processSource(source.join("|"));
        };
        /**
         * 检查自身属性数据信息的完整性。
         * @return {Boolean} 检查结果。
    ...
    var CustomVariable = function(config, domainHash, gaCookie, cache){
        ...
        oThis.initialize = function(){
            var _variables;
            _variables = Pick(oThis.gaCookie.getCookie(), _param_utmv + _domainHash + ".", ";").split(_domainHash + ".")[1];
            if(!IsEmpty(_variables)){
                ...
                if(!IsEmpty(_var_1)){
                    _var_1 = _var_1.split(",");
                    for(var i = 0; i < _var_1.length; i++){
                        _variable = _var_1[i];
                        if(!IsEmpty(_variable)){
                            _variable = _variable.split(_equal);
                            if(_variable.length == 4){
         ...
         * @param index {Number} 必填。 自定义变量所用的位置。可能的值有 1-5（包括 1 和 5）。
         * @param name {String} 必填。 自定义变量的名称。
         * @param value {String} 必填。 自定义变量的值。
         * @param scope {Number} 可选。自定义变量所使用的范围。可能的值包括 1（访问者级）、2（会话级）和 3（网页级）。
        ...
        oThis.setCustomVar = function(index, name, value, scope){
            ...
            if(name && value && index > 0 && index <= _config.maxCustomVariables){
                name = Encode(name);
                value = Encode(value);
                if(name.length + value.length <= 64){
        ...
        /**
         * 返回分配给指定索引的访问者级自定义变量。
         * @param index {Number} 访问者级自定义变量的索引。
         ...
         * @param index {Number} 要删除的自定义变量索引。
         */
    ...
    var Cache = function(){
        ...
        function _set(cache, key, idx, vlaue){
            if(undefined == _Cache[cache]){
                _Cache[cache] = {};
            }
            if(undefined == _Cache[cache][key]){
                _Cache[cache][key] = [];
            }
        ...
        function _get(cache, key, idx){
            if(undefined != _Cache[cache] && undefined != _Cache[cache][key]){
        ...
        function _clear(cache, key){
            if(undefined != _Cache[cache] && undefined != _Cache[cache][key]){
                _Cache[cache][key] = undefined;
                var y = true,
                    i;
                for(i = 0; i < _map_key.length; i++){
                    if(undefined != _Cache[cache][_map_key[i]]){
                        y = false;
                        break;
                    }
                }
        ...
        function _serialize(o){
            ...
            for(i = 0; i < _map_key.length; i++){
                ...
                if(undefined != _key){
                    ...
                    for(G = 0; G < _key.length; G++){
                        if(undefined != _key[G]){
                            J = "";
                            if(G != t && undefined == _key[G - 1]){
                                J += G.toString() + _exclamation;
                            }
                            var R;
                            R = _key[G];
                    ...
                    _key = _left_parenthesis + y.join(_asterisk) + _right_parenthesis;
                    serialization += _key;
                    y = false;
                }else{
                    y = true;
        ...
        oThis.serialize = function(){
            ...
            for(cache in _Cache){
                if(undefined != _Cache[cache]){
                    serialization += cache.toString() + _serialize(_Cache[cache]);
                }
            }
            return serialization;
        };
        ...
        oThis.Serialize = function(cache){
            ...
            var serialization = cache.serialize(),
                key;
            for(key in _Cache){
                if(undefined != _Cache[key] && !cache.exist(key)){
                    serialization += key.toString() + _serialize(_Cache[key]);
                }
            }
            return serialization;
        };
        ...
        oThis.setValue = bind("_setValue", 90, function(cache, idx, value){
            if(typeof value != "number" && (undefined == Number || !(value instanceof Number)) || Math.round(value) != value || value == NaN || value == Infinity){
                return false;
            }
            _set(cache, _value, idx, value.toString());
            return true;
        });
        /**
         * 获取指定缓存索引的键。
	...
	var TimeTracker = function(tracker, gaAjax){
	    var oThis = this,
	        external = $Global.window.external,
	        webkitPerformance = $Global.window.webkitPerformance,
	        _trackTimeRate = 10;
	    ...
	    oThis.getLoadTime = function(){
	        var i,
	            _timing = "timing",
	            _onloadT = "onloadT";
	        if(external && external[_onloadT] != undefined && external.isValidLoadTime){
	            i = external[_onloadT];
	    ...
	    oThis.send = function(){
	        ...
	        gaAjax.send(param, tracker.account, tracker.domainHash, false, true);
	    };
	    /**
	     * 跟踪加载时间。
	     */
	    oThis.trackLoadTime = function(){
	        ...
	        external && external.isValidLoadTime != undefined && external.setPageReadyTime();
	        return false;
	    };
	    /**
	     * 跟踪页面加载时间。
	    ...
	    oThis.trackPageLoadTime = function(){
	        ...
	        oThis.trackLoadTime() && AddListener($Global.window, "load", oThis.trackLoadTime, false);
	        return true;
	    };
	};
	...
	GASO.getGASO = function(gaCookie){
	    var _gaso = "gaso=",
	        result = "",
	        url = $Global.document.location.hash;
	    if(url && 1 == url.indexOf(_gaso)){
	        result = Pick(url, _gaso, _amp_);
	    }else{
	        result = (url = $Global.window.name) && 0 <= url.indexOf(_gaso) ? Pick(url, _gaso, _amp_) : Pick(gaCookie.getCookie(), _param_gaso, ";");
	    }
	    return result;
	};
	/**
	...
	GASO.create = function(gaso, domain){
	    ...
	    ($Global.document.getElementsByTagName("head")[0] || $Global.document.getElementsByTagName("body")[0]).appendChild(script);
	};
	/**
	 * 加载GASO令牌请求。
	 * @param config {Config} 配置信息。
	...
	var Tracker = function(name, account, namespace){
	    ...
	    function _notGoogleDomain(){
	        ...
	        var domain = config.domain,
	            _isGoogle = domain.indexOf("www.google.") * domain.indexOf(".google.") * domain.indexOf("google.");
	        return _isGoogle || ("/" != config.cookiePath) || (domain.indexOf("google.org") > -1);
	    }
	    /**
	     * 设置MAP字典的时间戳。
	     * @param map {Object} MAP字典。
	    ...
	    function _isIgnoreUrl(){
	        return ("file:" != $Global.document.location.protocol) && _notGoogleDomain();
	    }
	    var oThis = this,
	        bind = CreateBinder(oThis),
	        gaAjax = undefined,
	    ...
	    oThis.name = name;
	    oThis.timestamp = Math.round((new Date).getTime()/1E3);
	    /**
	     * UA 编号，网络载体 ID。
	     */
	    oThis.account = account || "UA-XXXXX-YY";//XXXXX：帐户编号；YY：帐户内的配置文件编号。
	    oThis.referrer = $Global.document.referrer;
	    oThis.processedSource = undefined;
	    oThis.transactions = undefined;
	    oThis.A = false;
	    oThis.client = undefined;
	    oThis.cache = undefined;
	    oThis.eventCache = undefined;
	    oThis.pageLoadTimeTracker = undefined;
	    oThis.domainHash = undefined;
	    oThis.gaCookie = undefined;
	    config.namespace = namespace ? Encode(namespace) : undefined;
	    /**
	     * 生成访问者ID。
	     * @return {String} 访问者ID。
	     */
	    ...
	    oThis.processReferrer = function(referrer, domain){
	        if(IsEmpty(referrer)){
	            ...
	            var n = referrer.indexOf(domain);
	            referrer = (n >= 0 && n <= 8) ? "0" : (("[" == referrer.charAt(0)) && ("]" == referrer.charAt(referrer.length - 1))) ? "-" : referrer;
	        }
	        return referrer;
	    };
	    /**
	    ...
	    oThis.getPageInfo = function(path){
	        ...
	        param += (config.detectTitle && !IsEmpty($Global.document.title)) ? "&utmdt=" + Encode($Global.document.title) : "";
	        var hid;
	        hid = undefined;
	        if($Global.window && $Global.window.gaGlobal && $Global.window.gaGlobal.hid){
	            hid = $Global.window.gaGlobal.hid;
	        }else{
	            hid = Random();
	            $Global.window.gaGlobal = $Global.window.gaGlobal ? $Global.window.gaGlobal : {};
	            $Global.window.gaGlobal.hid = hid;
	        }
	        param += "&utmhid=" + hid + "&utmr=" + Encode(String(oThis.processedSource)) + "&utmp=" + Encode(oThis.processPath(path));
	        return param;
	    };
	    /**
	     * 处理页面路径。
	    ...
	    oThis.processPath = function(path){
	        var location = $Global.document.location;
	        path && Register(13);
	        return path = ((undefined != path) && ("" != path)) ? Encode(path, true) : Encode(location.pathname + location.search, true);
	    };
	    /**
	     * 发送跟踪请求。
	     * @param path {String} 页面路径。
	    ...
	    oThis.send = function(path){
	        if(oThis.isSample()){
	            var param = "";
	            if(oThis.cache != undefined && oThis.cache.serialize().length > 0){
	    ...
	    oThis.getParams = function(){
	        ...
	        return _gaCookie.setDomainHashByCookie(oThis.domainHash) ? _gaCookie.getParams() : undefined;
	    };
	    /**
	    ...
	    oThis.getLinkerUrl = bind("_getLinkerUrl", 52, function(targetUrl, useHash){
	        ...
	        if(params){
	            if(useHash && (1 >= n.length)){
	                _targetUrl += "#" + params;
	            }
	        }else if(!useHash || (1 >= n.length)){
	            if(1 >= n.length){
	    ...
	    oThis.rc = function(){
	        var timestamp = oThis.timestamp,
	            ...
	            gaGlobal = $Global.window ? $Global.window.gaGlobal : undefined,
	            _anchor,
	        ...
	        if(config.allowLinker){
	            _anchor = ($Global.document.location && $Global.document.location.hash) ? $Global.document.location.href.substring($Global.document.location.href.indexOf("#")) : "";
	            if(config.allowAnchor && !IsEmpty(_anchor)){
	                param = _anchor + _amp_;
	            }
	            param += $Global.document.location.search;
	        ...
	        if(IsEmpty(_utma)){
	            if(_has_utma){
	                if(_utma = (!_has_utmb || !_has_utmc)){
	                    _utma = _setMapTimestamp(cookie, ";", String(timestamp));
	                    oThis.A = true;
	                }else{
	                    _utma = Pick(cookie, _param_utma + domainHash + ".", ";");
	                    _utmb = Pick(cookie, _param_utmb + domainHash, ";").split(".");
	                }
	            }else{
	                _utma = [domainHash, oThis.getVisitorId(), timestamp, timestamp, timestamp, 1].join(".");
	                ma = oThis.A = true;
	            }
	        }else if(IsEmpty(_gaCookie.getUTMB()) || IsEmpty(_gaCookie.getUTMC())){
	            _utma = _setMapTimestamp(param, _amp_, String(timestamp));
	            oThis.A = true;
	        }else{
	            _utmb = _gaCookie.getUTMB().split(".");
	            domainHash = _utmb[0];
	        }
	        _utma = _utma.split(".");
	        if($Global.window && gaGlobal && gaGlobal.dh == domainHash && !config.namespace){
	            _utma[4] = gaGlobal.sid ? gaGlobal.sid : _utma[4];
	            if(ma){
	                _utma[3] = gaGlobal.sid ? gaGlobal.sid : _utma[4];
	                if(gaGlobal.vid){
	                    timestamp = gaGlobal.vid.split(".");
	                    _utma[1] = timestamp[0];
	                    _utma[2] = timestamp[1];
	                }
	            }
	        }
	        _gaCookie.setUTMA(_utma.join("."));
	        _utmb[0] = domainHash;
	        _utmb[1] = _utmb[1] ? _utmb[1] : 0;
	        _utmb[2] = undefined != _utmb[2] ? _utmb[2] : config.Sc;
	        _utmb[3] = _utmb[3] ? _utmb[3] : _utma[4];
	        _gaCookie.setUTMB(_utmb.join("."));
	    ...
	    oThis.initData = bind("_initData", 2, function(){
	            ...
	            if(!d){
	                ...
	                oThis.cache = new Cache;
	                customVariable = new CustomVariable(config, String(oThis.domainHash), oThis.gaCookie, oThis.cache);
	                oThis.setGaAjax();
	            }
	            if(_isIgnoreUrl()){
	                if(!d){
	                    oThis.processedSource = oThis.processReferrer(oThis.referrer, $Global.document.domain);
	                    sourceTracker = new SourceTracker(String(oThis.domainHash), oThis.processedSource, oThis.timestamp, config);
	                }
	    ...
	    oThis.Uc = function(){
	        ...
	        if(Random() % 1E3 === 42){
	            try{
	            	if(win.external && win.external.onloadT != undefined || win.webkitPerformance && win.webkitPerformance.timing){
	                    Register(12);
	                }
	            }catch(ex){}
	        }
	    ...
	    oThis.trackTrans = bind("_trackTrans", 18, function(){
	            ...
	            if(oThis.transactions && oThis.isSample()){
	                for(i = 0; i < oThis.transactions.transactions.length; i++){
	                    tran = oThis.transactions.transactions[i];
	                    Push(params, tran.serialize());
	                    for(j = 0; j < tran.items.length; j++){
	                        Push(params, tran.items[j].serialize());
	                    }
	                }
	                for(i= 0; i < params.length; i++){
	                    gaAjax.send(params[i], oThis.account, domainHash, true);
	                }
	            }
	        });
	    /**
	    ...
	    oThis.setTrans = bind("_setTrans", 20, function(){
	            var oTrans,
	                ...
	                j,
	                transactions;
	            oTrans = $Global.document.getElementById ? $Global.document.getElementById("utmtrans") : $Global.document.utmform && $Global.document.utmform.utmtrans ? $Global.document.utmform.utmtrans : undefined;
	            oThis.initData();
	            if(oTrans && oTrans.value){
	                oThis.transactions = new GaEComm;
	                transactions = oTrans.value.split("UTM:");
	                ...
	                for(i = 0; i < transactions.length; i++){
	                    ...
	                    items = transactions[i].split(config.transactionDelim);
	    ...
	    oThis.link = bind("_link", 101, function(targetUrl, useHash){
	            if(config.allowLinker && targetUrl){
	                oThis.initData();
	                $Global.document.location.href = oThis.getLinkerUrl(targetUrl, useHash);
	            }
	        });
	    /**
	     * 此方法与 _setDomainName() 和 _setAllowLinker() 方法一起使用可以启用跨域用户跟踪。
	    ...
	    oThis.sendXEvent = bind("_sendXEvent", 78, function(cache){
	        ...
	        if(oThis.isSample()){
	            ...
	            gaAjax.send(param, oThis.account, oThis.domainHash, false, true);
	        }
	    });
	    /**
	     * 创建事件跟踪器对象实例。
	    ...
	    oThis.trackEvent = bind("_trackEvent", 4, function(category, action, label, value){
	            ...
	            if(undefined != category && undefined != action && "" != category && "" != action){
	                _eventCache.clearKey(5);
	                _eventCache.clearValue(5);
	                (category = _eventCache.setKey(5, 1, category) && _eventCache.setKey(5, 2, action) && (undefined == label || _eventCache.setKey(5, 3, label)) && (undefined == value || _eventCache.setValue(5, 1, value))) && oThis.sendXEvent(_eventCache);
	            }else{
	                category = false;
	            }
	            return category;
	    ...
	    oThis.addOrganic = bind("_addOrganic", 14, function(newOrganicEngine, newOrganicKeyword, prepend){
	            config.organic.splice(prepend ? 0 : config.organic.length, 0, new Organic(newOrganicEngine, newOrganicKeyword));
	        });
	    /**
	...
	var TrackerFactory = function(){
	     ...
	     * 是否匿名化处理跟踪器对象发送的信息。
	     */
	    oThis.anonymizeIp = false;
	    oThis.Trackers = {};
	    oThis.Counter = 0;
	    oThis._gasoDomain = undefined;
	    oThis._gasoCPath = undefined;
	    /**                    	
	    ...
	    oThis.getTracker = bind("_getTracker", 0, function(account, namespace){
	            return oThis.createTracker(account, undefined, namespace);
	        });
	    /**
	    ...
	    oThis.getTrackerByName = bind("_getTrackerByName", 51, function(name){
	            name = name || "";
	            return _TrackerFactory.Trackers[name] || _TrackerFactory.createTracker(undefined, name);
	        });
	    /**
	     */
	    oThis.getGaUserPrefs = function(){
	            var gaUserPrefs = window[_gaUserPrefs_];
	            ...
	            try{
	                if(typeof _arguments[i] === "function"){
	                    ...
	                    var name = "",
	                        ...
	                        idx = o.lastIndexOf(".");
	                    if(idx > 0){
	                        name = o.substring(0, idx);
	                        func = o.substring(idx + 1);
	                    }
	...
	_tracker_ : {
	    ...
	    window[_gaq_] = _AsyncTrackerFactory;
	    isArray && _AsyncTrackerFactory.push.apply(_AsyncTrackerFactory, gaq);
	}
})();          	